---
title: "2023_Section_2_Practice"
author: "Francis Santiago & Yikun Chi"
date: "2023-10-13"
output: 
  html_document:
      toc: true
...
# Previously in Comm 106 Sections 
...
In R, we can create object to store specific values (e.g., characters, a number, a boolean value). We can print the the value of the object stores as well as showing what kind of value the object stores. 

- Note assigning existing object a new value will rewrite the existing object 
...
# TA Demo: Attempt to calculate the opposite of mystery_object and explain the error 
...
- To read and write files, you need to tell R where it should look
...
- You can achieve the same thing in the Files interface on your right

```{r echo = TRUE}

# TA Demo: Find current working directory 

# TA Demo: Set working directory to a folder a folder called section1

# TA Demo: Check the update

# TA Demo: Switch to previous level of directory, create a new folder called section2, and set it as working directory using the UI interface 
...
knitr::opts_knit$set(root.dir = "/Users/Serena/Desktop/section1")
```


## Loading Data
- R allows you to load datafiles from your computer (or online storages)
- Datafiles are usually stored as `data.frame` objects
- Use the function `read_csv()` to read a file in CSV format

```{r}
library(tidyverse)
# TA DEMO: load file "women.csv" using read_csv (from tidyverse) into df.women
# Make sure "women.csv" is in your working directory first!
```
...
```{r}
# TA DEMO let's add something to the dataframe, e.g. mean height
#df.women$mean_height <- mean(df.women$height, na.rm = TRUE)

# TA DEMO now it's a different data frame
#colnames(df.women)

# TA DEMO: so let's save it as "df.women_new.csv"
```
...
# Working with Dataframe
We just explored how to use R to read in data and write data to your computer. Now let's explore  more ways to use dataframes. 

## Summary Information 
You can use `summary` function to view the summary information of the dataframe. Here we use a more complicated dataset about passenger of Titanic. You can find out more information about this dataset from [here](https://www.kaggle.com/competitions/titanic/data)
...
Recall that object has class (e.g., boolean, number, character). Each column of your dataframe is essentially a vector(a collection of object of the same class). As a result, they also have class You can find out the type of columns using `str()`
```{r}
...
- Here we use `pull()` operation and `mean(na.rm=TRUE)`. We will revisit these later in more detail. At high level, `pull` pulls the result into a vector, which then allows you to calcualte the average with `mean` operation. The `na.rm = TRUE` within `mean` tells the function to ignore all the rows where the record is missing. 
```{r}
